...
# Date: 02.08.2022
# Subject: Fine-tuning and testing of BERT-BiLSTM-CRF model via Hugging Face Trainer API and Transformers library
...
class BERT_BiLSTM_CRF_MODEL:
    ...
    def train(self):
        ...
        model = BERT_BiLSTM_CRF(pretrained_path='dbmdz/bert-base-turkish-cased', num_labels=len(self.label2id))
        ...
        trainer = BERT_BiLSTM_CRF_Trainer(model=model,
                                          args=training_args,
        ...
        torch.save(model.state_dict(), self.model_path + 'berturk_bilstm_crf')
    ...
    def evaluate(self, result_path):
        ...
        model = BERT_BiLSTM_CRF(pretrained_path='dbmdz/bert-base-turkish-cased', num_labels=len(self.label2id))
        model.load_state_dict(torch.load(self.model_path + 'berturk_bilstm_crf'))
        trainer = BERT_BiLSTM_CRF_Trainer(args=training_args, model=model, callbacks=[ProgressCallback])
...
class BERT_BiLSTM_CRF(nn.Module):
    def __init__(self, pretrained_path='dbmdz/bert-base-turkish-cased',
                 config='dbmdz/bert-base-turkish-cased',
                 num_labels=7):
...
class BERT_BiLSTM_CRF_Trainer(Trainer):
