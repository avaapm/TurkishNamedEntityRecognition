...
# Date    : 14.07.2022
# Subject : BERT fine-tuning with CRF layer at the top of BERT architecture using Transformers library
...
from transformers import AutoTokenizer, AutoModelForTokenClassification, BertModel, Trainer, \
...
class BERT_CRF(nn.Module):
    def __init__(self, pretrained_path='dbmdz/bert-base-turkish-cased', num_labels=7):
...
for path in paths:
    ...
    recall = []
    f_scores = []
    if not os.path.isdir(path[1] + '/bert_crf'):
        os.mkdir(path[1] + '/' + '/bert_crf')
    ...
    for rep in range(10):
        ...
        data_handler = CoNLLReader(
            path[0] + 'train_CONLL.txt',
            path[0] + 'test_CONLL.txt')
        ...
        model = BERT_CRF(pretrained_path='dbmdz/bert-base-turkish-cased', num_labels=len(label2id))
        ...
        class_report = path[1] + '/bert_crf/entities_repeat_' + str(rep)
        with open(class_report, "w") as text_file:
            text_file.write(report)

        pred = path[1] + '/bert_crf/pred_repeat_' + str(rep)
        with open(pred, 'w') as f:
            wr = csv.writer(f, delimiter=' ')
            wr.writerows(post_pred)

        if rep == 0:
            test_set = path[1] + '/bert_crf/test_set'
    ...
    result_txt = path[1] + '/bert_crf/FINAL_RESULTS'
